...
<head>
    ...
    <link rel="stylesheet" href="<?php echo BASE_URL ?>css/vmb.css?v=3">
...
<body>
                                                ...
                                                <div id="popup" class="popup">
                                                    <div class="popup-content">
                                                        ...
                                                        <div style="display: flex; justify-content: center;align-items: center;">
                                                            <button type="button" class="btn btn-sm bg-light my-2 mx-1 py-1" id="showhoso" data-bs-toggle="modal" data-bs-target="#exampleModal">Xem hồ sơ</button>
                                                            <button type="button" class="btn btn-sm bg-light my-2 mx-1 py-1 "><a class="text-decoration-none text-dark d-inline" href="<?php echo BASE_URL ?>login/loadedituser/<?php echo Session::get('nguoidung_id'); ?>">Sửa hồ sơ</a></button>
                                                        </div>
                                                        <hr>
                        ...
                        <form class="form-banner" action="">
                            <div class="input-icon-1">
                                <div class="input-icon" >
                                    ...
                                    <input class="icon-address-1" id="destination" type="text" placeholder="Bạn muốn đi đâu?">
                                </div>
                                <div class="input-icon">
                                    <span class="icon-01 material-symbols-outlined" style="line-height: 0.5; color: #dfdcdc;">
                                        person_search
                                    </span>
                                    <input class="icon-time-1" type="number" id="number_of_people" placeholder="Số người?" >
                                </div>
    ...
    <div class="container">
        <h2 class="mt-3">Khuyến Mãi Và Ưu Đãi</h2>
        <h5 class="">Tận hưởng ưu đãi - Trãi nghiệm thoải mái</h5>
        <div id="demo" class="carousel slide" data-bs-ride="carousel">
            <div class="carousel-inner">
                <div class="carousel-item active">
                    <img src="<?php echo BASE_URL ?>img/banner-flight.webp" class="d-block" style="width:100%">
                </div>
                <div class="carousel-item">
                    <img src="<?php echo BASE_URL ?>img/banner-flight1.webp" class="d-block" style="width:100%">
                </div>
                <div class="carousel-item">
                    <img src="<?php echo BASE_URL ?>img/banner-flight2.webp" class="d-block" style="width:100%">
                </div>
            </div>
        ...
        <ul class="row list-unstyled gy-3 " style="margin: 0 auto;">
            <?php
            foreach ($listflights as $key => $value) {
            ?>
                <li class="col-lg-3 col-sm-6">
                    <div class="plane-ticket ">
                        <img src="<?php echo BASE_URL ?><?php echo htmlspecialchars($value['imgvmb'], ENT_QUOTES, 'UTF-8'); ?>">

                        <div class="plane-ticket-body">
                            <div class="plane-ticket-from">
                                <p><?php echo htmlspecialchars($value['noidi'], ENT_QUOTES, 'UTF-8'); ?> đến</p>
                            </div>
                            <div class="plane-ticket-to">
                                <p><?php echo htmlspecialchars($value['noiden'], ENT_QUOTES, 'UTF-8'); ?> </p>
                            </div>
                            <div class="plane-ticket-day">
                                <p><?php echo htmlspecialchars($value['thoigian'], ENT_QUOTES, 'UTF-8'); ?></p>
                            </div>
                            <div class="plane-ticket-titel">
                                <p><?php echo htmlspecialchars(number_format($value['giave'], 0, ',', '.'), ENT_QUOTES, 'UTF-8'); ?> VND</p>

                            </div>
                            <div class="plane-ticket-text">
                                <p><?php echo htmlspecialchars($value['loaive'], ENT_QUOTES, 'UTF-8'); ?></p>
                            </div>
                        </div>
                    </div>
                </li>
        ...
        <h2 class="mt-3">Kế hoạch riêng, giá đặc biệt</h2>
    ...
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
     ...
     Thêm jQuery UI cho Autocomplete và Datepicker 
    ...
    <script>
        $(document).ready(function() {
            // Danh sách địa điểm
            const locations = [
                "Hà Nội", "Sài Gòn", "Đà Nẵng", "Nha Trang", "Hạ Long", "Phú Quốc"
            ];

            // Kích hoạt Autocomplete cho ô "Bạn muốn đi đâu?"
            $("#destination").autocomplete({
                source: locations, // Dữ liệu nguồn
                minLength: 0, // Hiện gợi ý ngay khi click vào input, không cần nhập ký tự
                focus: function(event, ui) {
                    // Khi người dùng chọn một địa điểm từ gợi ý, giữ giá trị địa điểm đó trong ô input
                    $(this).val(ui.item.value);
                    return false;
                },
                select: function(event, ui) {
                    // Khi người dùng chọn một địa điểm, gán giá trị cho ô input
                    $(this).val(ui.item.value);
                    return false;
                }
            });

            // Kích hoạt Autocomplete cho ô "Khởi hành từ"
            $("#departure_location").autocomplete({
                source: locations,
                minLength: 0, // Hiện gợi ý ngay khi click vào input
                focus: function(event, ui) {
                    $(this).val(ui.item.value);
                    return false;
                },
            ...
            $('#increase').click(function() {
                let value = parseInt($('#number_of_people').val());
                $('#number_of_people').val(value + 1);
            });
            ...
            // Datepicker cho "Thời gian khởi hành"
            $("#departure_date").datepicker({
                dateFormat: "dd/mm/yy",
                minDate: 0 // Không cho phép chọn ngày trước ngày hôm nay
            });
        });
    </script> -->
